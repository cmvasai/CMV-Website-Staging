...
const EventDetails = ({ featuredEvents }) => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const fetchEventDetails = () => {
            try {
                ...
                const foundEvent = featuredEvents.find(e => e._id === id);
    ...
    const handleShare = async () => {
        ...
        const shareData = {
            title: event.name || 'Chinmaya Mission Event',
    ...
    const lightboxImages = event.images?.map((img, index) => ({
        ...
        caption: typeof img === 'string' ? `${event.name} - Photo ${index + 1}` : (img.caption || `${event.name} - Photo ${index + 1}`)
    })) || [];
    ...
    return (
        <>
            <Helmet>
                <title>{event.name} | Events | Chinmaya Mission Vasai</title>
                <meta name="description" content={event.description} />
                <meta name="keywords" content={`${event.name}, event, Chinmaya Mission Vasai`} />
